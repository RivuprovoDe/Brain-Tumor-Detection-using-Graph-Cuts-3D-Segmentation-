...
#      python train.py
#
#  Before running, open config.py and set BRATS_ROOT to your dataset folder.
# =============================================================================
...
def validate(model, val_dataset, device, patch_size, stride):
    ...
    Returns mean Dice over the validation subjects.
    """
    model.eval()
    dice_scores = []
    ...
    with torch.no_grad():
        for idx in range(len(val_dataset)):
            volume4d, label = val_dataset[idx]   # (4,D,H,W), (1,D,H,W)
            volume_np = volume4d.numpy()          # (4,D,H,W)
            ...
            prob_vol = stitch_patches(prob_patches, (D, H, W), patch_size)
            pred_t   = torch.from_numpy(prob_vol).unsqueeze(0).unsqueeze(0)
            dice     = dice_score(pred_t, label.unsqueeze(0))
...
def main():
    ...
    train_ds = BraTSDataset(train_subs, mode='train',
                             patch_size=cfg.PATCH_SIZE, patches_per_volume=4)
    val_ds   = BraTSDataset(val_subs,   mode='val',
                             patch_size=cfg.PATCH_SIZE)

    # Save test subject list for later use in predict.py
