...
def validate(model, val_dataset, device, patch_size, stride):
    ...
    with torch.no_grad():
        for idx in range(len(val_dataset)):
            ...
            C, D, H, W = volume_np.shape
            pd, ph, pw = patch_size

            positions = [
                (max(0,(D-pd)//4),   max(0,(H-ph)//4),   max(0,(W-pw)//4)),
                (max(0,(D-pd)//2),   max(0,(H-ph)//2),   max(0,(W-pw)//2)),
                (max(0,3*(D-pd)//4), max(0,3*(H-ph)//4), max(0,3*(W-pw)//4)),
            ]
            ...
            for d0, h0, w0 in positions:
                ...
                patch = volume_np[:, d0:d0+pd, h0:h0+ph, w0:w0+pw]
                lbl   = label.numpy()[0, d0:d0+pd, h0:h0+ph, w0:w0+pw]

                def pad_to(arr, target):
                    pads = [(0, max(0, t-s)) for s, t in zip(arr.shape, target)]
                    return np.pad(arr, pads, mode='constant')

                patch = pad_to(patch, (C, pd, ph, pw))
                lbl   = pad_to(lbl,   (pd, ph, pw))

                patch_t = torch.from_numpy(patch).unsqueeze(0).to(device)
                prob    = torch.sigmoid(model(patch_t)).squeeze().cpu()
                pred_t  = prob.unsqueeze(0).unsqueeze(0)
                lbl_t   = torch.from_numpy(lbl).unsqueeze(0).unsqueeze(0)
                vol_dices.append(dice_score(pred_t, lbl_t))

            dice_scores.append(float(np.mean(vol_dices)))
